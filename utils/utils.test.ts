...
Deno.test("Should handle strings without any params", () => {
  assertEquals(
    "here is no params",
    extractValuesInTemplate("here is no params", {}),
  );
  assertEquals(
    "here is no params {{",
    extractValuesInTemplate("here is no params {{", {}),
  ...
  assertEquals(
    "here is no params {{a",
    extractValuesInTemplate("here is no params {{a", {}),
  );
  assertEquals(
    "here is no params {{ a}",
    extractValuesInTemplate("here is no params {{ a}", {}),
...
Deno.test("Should just print the var if no data is found", () => {
  assertEquals(
    "params without {{data}}",
    extractValuesInTemplate("params without {{data}}", {}),
  );
  assertEquals(
    "params without {{ data}}",
    extractValuesInTemplate("params without {{ data}}", {}),
...
Deno.test("Should extract values if found in data (no depth)", () => {
  assertEquals(
    "params without 1",
    extractValuesInTemplate("params without {{data}}", { data: 1 }),
  );
  assertEquals(
    "params without 1",
    extractValuesInTemplate("params without {{ data}}", { data: "1" }),
  ...
  assertEquals(
    "params without 2",
    extractValuesInTemplate("params without {{ data }}", { data: 1 + 1 }),
  );
});

Deno.test("Should extract values if found in data (with depth)", () => {
  assertEquals(
    "My name is Johannes, I work at Consid",
    extractValuesInTemplate(
      "My name is {{person.name}}, I work at {{ person.work}}",
      { person: { name: "Johannes", work: "Consid" } },
    ),
  );
});
...
Deno.test("flatten objects paths with objects", () => {
  ...
  assertEquals(
    { "a.b": true, "a.c": false },
    flattenObject({ a: { b: true, c: false } }),
  );
  assertEquals(
    { "person.name": "johannes", "person.work": "consid" },
    flattenObject({ person: { name: "johannes", work: "consid" } }),
  );
});
...
Deno.test("flattenObjectWithArray", () => {
  assertEquals(
    { "items[0].name": "hi there" },
    flattenObject({ items: [{ name: "hi there" }] }),
  );
  assertEquals(
    { "a.b": true, "a.c[0]": false, "a.c[1]": true },
    flattenObject({ a: { b: true, c: [false, true] } }),
  );
});
